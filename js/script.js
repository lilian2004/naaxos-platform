...
function openNews(id) {
  ...
  // SÉCURITÉ : les métadonnées d'article (tag/titre/intro/cta) peuvent venir de
  // la base Supabase → on les échappe avant injection (défense en profondeur).
  // (Le corps des sections est déjà échappé à la source pour les articles Supabase.)
  document.getElementById('newsModalContent').innerHTML =
    '<div class="news-modal-tag"><span class="news-tag">' + _escapeHtml(safeTag) + '</span> · <span class="news-date">' + _escapeHtml(safeDate) + '</span></div>' +
    '<h2>' + _escapeHtml(safeTitle) + '</h2>' +
    (safeImage ? '<img src="' + _escapeHtml(safeImage) + '" alt="' + _escapeHtml(safeTitle) + '" class="news-modal-img" loading="lazy">' : '') +
    (safeIntro ? '<p><em>' + _escapeHtml(safeIntro) + '</em></p>' : '') +
    sectionsHtml +
    '<div style="margin-top:28px;padding:20px 24px;background:rgba(112,131,224,0.07);border-radius:10px;border-left:3px solid var(--secondary);">' +
      '<p style="font-weight:600;color:var(--primary);margin-bottom:12px;">' + _escapeHtml(safeCta) + '</p>' +
...
(function() {
  ...
  // ╔══════════════════════════════════════════════════════════════════╗
  // ║  CHATBOT LLM — GUIDE DE CONFIGURATION (via proxy backend)        ║
  // ╠══════════════════════════════════════════════════════════════════╣
  // ║                                                                  ║
  // ║  La clé Groq n'est JAMAIS mise dans ce code (elle serait visible ║
  // ║  par tous les visiteurs et réutilisable → facturée sur ton compte║
  // ║  Le site appelle l'Edge Function "chat-proxy", qui détient la clé║
  // ║  côté serveur et parle à Groq pour nous.                         ║
  // ║                                                                  ║
  // ║  POUR ACTIVER LE CHAT :                                          ║
  // ║   1. Créer une clé Groq : https://console.groq.com/keys         ║
  // ║   2. La stocker comme secret (jamais dans le code) :            ║
  // ║        supabase secrets set GROQ_API_KEY=gsk_xxx                ║
  // ║   3. Déployer le proxy :                                         ║
  // ║        supabase functions deploy chat-proxy                     ║
  // ║   4. Passer enabled à true ci-dessous.                          ║
  // ║                                                                  ║
  // ║  Pour désactiver : enabled: false → le chatbot retombe sur les  ║
  // ║  menus pré-définis (FLOWS).                                      ║
  // ╚══════════════════════════════════════════════════════════════════╝
  const LLM_CONFIG = {
    enabled:  false,                 // true = activer le chat IA (après déploiement du proxy)
    provider: 'groq',
    // URL du proxy backend (la clé reste côté serveur). Reprise depuis config.js.
    fnUrl:    (window.NAAXOS_CONFIG && window.NAAXOS_CONFIG.CHAT_FN_URL) || ''
  };
...
7. Quand l'information n'est pas certaine, dis-le clairement et propose un rendez-vous.`;
  ...
  function addUserMsg(text) {
    ...
    // SÉCURITÉ : le texte vient de l'utilisateur → on l'échappe pour bloquer toute injection HTML/JS (XSS)
    row.innerHTML = '<div class="msg-bbl">' + _escapeHtml(text) + '</div>';
    msgs_el.appendChild(row);
    scrollChat();
  }
  ...
  async function callLLM(userMessage) {
    if (!LLM_CONFIG.enabled || !LLM_CONFIG.fnUrl) {
  ...
  // ── Groq (via proxy backend — la clé reste côté serveur) ─────────────
  // SÉCURITÉ : on n'appelle plus api.groq.com directement (sinon la clé
  // GROQ_API_KEY serait visible dans le navigateur). On passe par l'Edge
  // Function "chat-proxy" qui détient la clé et appelle Groq côté serveur.
  async function callGroq(message) {
    var fnUrl = (window.NAAXOS_CONFIG && window.NAAXOS_CONFIG.CHAT_FN_URL) || '';
    if (!fnUrl) throw new Error('CHAT_FN_URL non configurée');
    ...
    // Jeton anti-bot Turnstile : le proxy l'exige pour éviter d'être utilisé
    // comme passerelle LLM gratuite. Réutilise le widget existant (#chatTurnstile).
    // ⚠️ À l'activation du chat : ajouter dans le panneau de chat un élément
    //    <div class="naaxos-turnstile" id="chatTurnstile"></div> (idéalement en
    //    mode invisible) pour que getResponse renvoie un jeton.
    var token = (typeof _turnstileToken === 'function') ? _turnstileToken('chatTurnstile') : '';
    ...
    if (typeof _turnstileReset === 'function') _turnstileReset('chatTurnstile');
    if (!res.ok) throw new Error('Chat proxy HTTP ' + res.status);
    var data = await res.json();
    var text = data?.text;
  ...
  function handleLLMReply(raw, userMsg) {
    ...
    // Afficher la réponse
    // SÉCURITÉ : 'clean' vient du LLM (source non fiable) → on échappe le HTML
    // puis on ne réautorise que les sauts de ligne. Les messages prédéfinis
    // (FLOWS) restent en HTML riche car ils sont écrits par nous, pas par le LLM.
    addBotMsg(_escapeHtml(clean).replace(/\n+/g, '<br>'));
  ...
  window.sendMsg = function() {
    ...
    if (localReply && (!LLM_CONFIG.enabled || !LLM_CONFIG.fnUrl)) {
      addTyping();
      setTimeout(function() {
        removeTyping();
